...
public class MethodsAndFlow 
{
	...
	{
		this.outputSubset(0, this.stringArray.length - 1);
	}
	
	//check to make sure the provided parameters are within the bounds of the array and are positive numbers 
	public Boolean noErrorsFound()
	{
		String err = "";
		
		if(startPosition < 0) err += "Error: You entered a negative start position, please enter a positive start position.\n";
		if(startPosition > stringArray.length - 1) err += "Error: You entered a start position that is larger than the length of the array, please enter a smaller number.\n";
		if(startPosition + numberOfValuesToOutput > stringArray.length) err += "Error: You entered a start position and number of values to output that will run past the length of the array, enter a smaller combination.\n";
